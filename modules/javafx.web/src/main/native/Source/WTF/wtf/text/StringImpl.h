...
bool isAllSpecialCharacters(const CharacterType *, size_t length);
...
class StringImpl : private StringImplShape
{
    ...
    template <typename>
    friend struct WTF::BufferFromStaticDataTranslator;
    template <typename>
...
private:
    ...
    enum ConstructWithoutCopyingTag
    {
        ConstructWithoutCopying
    };
...
public:
    ...
    static Ref<StringImpl> create8BitIfPossible(const Vector<UChar, inlineCapacity> &);
    ...
    template <unsigned characterCount>
    static Ref<StringImpl> createFromLiteral(const char (&)[characterCount]);
    ...
    static RefPtr<StringImpl> tryCreateUninitialized(unsigned length, CharacterType *&);
    ...
    {
        return bufferOwnership() == BufferSubstring;
    }
#endif
...
public:
    ...
    public:
        ...
        template <unsigned characterCount>
        constexpr StaticStringImpl(const char (&characters)[characterCount], StringKind = StringNormal);
        template <unsigned characterCount>
        constexpr StaticStringImpl(const char16_t (&characters)[characterCount], StringKind = StringNormal);
        operator StringImpl &();
    };
    ...
    template <typename CharacterType>
    static void copyCharacters(CharacterType *destination, const CharacterType *source, unsigned numCharacters);
    ...
    bool startsWith(const char (&prefix)[matchLength]) const { return startsWith(prefix, matchLength - 1); }
    ...
    template <unsigned matchLength>
    bool endsWith(const char (&prefix)[matchLength]) const { return endsWith(prefix, matchLength - 1); }
    ...
    BufferOwnership bufferOwnership() const
    {
        return static_cast<BufferOwnership>(m_hashAndFlags & s_hashMaskBufferOwnership);
    }
...
private:
    ...
    template <CaseConvertType, typename CharacterType>
    static Ref<StringImpl> convertASCIICase(StringImpl &, const CharacterType *, unsigned);

    template <class CodeUnitPredicate>
    Ref<StringImpl> stripMatchedCharacters(CodeUnitPredicate);
    template <typename CharacterType>
    ALWAYS_INLINE Ref<StringImpl> removeCharacters(const CharacterType *characters, CodeUnitMatchFunction);
    template <typename CharacterType, class CodeUnitPredicate>
    Ref<StringImpl> simplifyMatchedCharactersToSpace(CodeUnitPredicate);
    template <typename CharacterType>
    static Ref<StringImpl> constructInternal(StringImpl &, unsigned);
    template <typename CharacterType>
    static Ref<StringImpl> createUninitializedInternal(unsigned, CharacterType *&);
    template <typename CharacterType>
    static Ref<StringImpl> createUninitializedInternalNonEmpty(unsigned, CharacterType *&);
    template <typename CharacterType>
    static Ref<StringImpl> reallocateInternal(Ref<StringImpl> &&, unsigned, CharacterType *&);
    template <typename CharacterType>
    static Ref<StringImpl> createInternal(const CharacterType *, unsigned);
...
template <typename CharacterType>
size_t reverseFindLineTerminator(const CharacterType *, unsigned length, unsigned index = std::numeric_limits<unsigned>::max());
template <typename CharacterType>
size_t reverseFind(const CharacterType *, unsigned length, CharacterType matchCharacter, unsigned index = std::numeric_limits<unsigned>::max());
...
template <size_t inlineCapacity>
bool equalIgnoringNullity(const Vector<UChar, inlineCapacity> &, StringImpl *);

template <typename CharacterType1, typename CharacterType2>
int codePointCompare(const CharacterType1 *, unsigned length1, const CharacterType2 *, unsigned length2);
...
template <typename CharacterType>
unsigned lengthOfNullTerminatedString(const CharacterType *);
...
template <>
ALWAYS_INLINE Ref<StringImpl> StringImpl::constructInternal<LChar>(StringImpl &string, unsigned length)
{
    return adoptRef(*new (NotNull, &string) StringImpl{length, Force8BitConstructor});
}

template <>
ALWAYS_INLINE Ref<StringImpl> StringImpl::constructInternal<UChar>(StringImpl &string, unsigned length)
{
    return adoptRef(*new (NotNull, &string) StringImpl{length});
}

template <>
ALWAYS_INLINE const LChar *StringImpl::characters<LChar>() const
{
    return characters8();
}

template <>
ALWAYS_INLINE const UChar *StringImpl::characters<UChar>() const
{
    return characters16();
}
...
template <typename CharacterType>
inline size_t reverseFindLineTerminator(const CharacterType *characters, unsigned length, unsigned index)
{
    if (!length)
        return notFound;
    if (index >= length)
...
template <typename CharacterType>
inline size_t reverseFind(const CharacterType *characters, unsigned length, CharacterType matchCharacter, unsigned index)
{
    if (!length)
        return notFound;
    if (index >= length)
...
template <size_t inlineCapacity>
inline bool equalIgnoringNullity(const Vector<UChar, inlineCapacity> &a, StringImpl *b)
{
...
inline StringImplShape::StringImplShape(unsigned refCount, unsigned length, const LChar *data8, unsigned hashAndFlags)
    : m_refCount(refCount), m_length(length), m_data8(data8), m_hashAndFlags(hashAndFlags)
{
}

inline StringImplShape::StringImplShape(unsigned refCount, unsigned length, const UChar *data16, unsigned hashAndFlags)
    : m_refCount(refCount), m_length(length), m_data16(data16), m_hashAndFlags(hashAndFlags)
{
}

template <unsigned characterCount>
inline constexpr StringImplShape::StringImplShape(unsigned refCount, unsigned length, const char (&characters)[characterCount], unsigned hashAndFlags, ConstructWithConstExprTag)
    : m_refCount(refCount), m_length(length), m_data8Char(characters), m_hashAndFlags(hashAndFlags)
{
}

template <unsigned characterCount>
inline constexpr StringImplShape::StringImplShape(unsigned refCount, unsigned length, const char16_t (&characters)[characterCount], unsigned hashAndFlags, ConstructWithConstExprTag)
    : m_refCount(refCount), m_length(length), m_data16Char(characters), m_hashAndFlags(hashAndFlags)
{
}
...
template <bool isSpecialCharacter(UChar)>
inline bool StringImpl::isAllSpecialCharacters() const
{
    if (is8Bit())
...
template <size_t inlineCapacity>
inline Ref<StringImpl> StringImpl::create8BitIfPossible(const Vector<UChar, inlineCapacity> &vector)
{
...
template <unsigned characterCount>
ALWAYS_INLINE Ref<StringImpl> StringImpl::createFromLiteral(const char (&characters)[characterCount])
{
...
template <typename CharacterType>
ALWAYS_INLINE RefPtr<StringImpl> StringImpl::tryCreateUninitialized(unsigned length, CharacterType *&output)
{
    if (!length)
    {
        fprintf(stderr, "[JSY] StringImpl::tryCreateUninitialized 1\n");
        output = nullptr;
        return empty();
    }
    ...
    {
        fprintf(stderr, "[JSY] StringImpl::tryCreateUninitialized 2\n");
        output = nullptr;
        return nullptr;
    }
    StringImpl *result;
    ...
    {
        fprintf(stderr, "[JSY] StringImpl::tryCreateUninitialized 3\n");
        output = nullptr;
        return nullptr;
    }
    output = result->tailPointer<CharacterType>();

    fprintf(stderr, "[JSY] StringImpl::tryCreateUninitialized 4\n");
...
inline Ref<StringImpl> StringImpl::adopt(Vector<CharacterType, inlineCapacity, OverflowHandler, minCapacity> &&vector)
{
    if (size_t size = vector.size())
    {
        ...
        {
            fprintf(stderr, "CRASHING from StringImpl.h:1096, size = %d\n", size);
            CRASH();
        }
...
template <typename CharacterType>
inline void StringImpl::copyCharacters(CharacterType *destination, const CharacterType *source, unsigned numCharacters)
{
    if (numCharacters == 1)
    {
        *destination = *source;
...
template <typename T>
inline size_t StringImpl::allocationSize(Checked<size_t> tailElementCount)
{
    return (tailOffset<T>() + tailElementCount * sizeof(T)).unsafeGet();
}

template <typename T>
inline size_t StringImpl::tailOffset()
{
#if COMPILER(MSVC)
...
template <unsigned characterCount>
inline constexpr StringImpl::StaticStringImpl::StaticStringImpl(const char (&characters)[characterCount], StringKind stringKind)
...
template <unsigned characterCount>
inline constexpr StringImpl::StaticStringImpl::StaticStringImpl(const char16_t (&characters)[characterCount], StringKind stringKind)
    : StringImplShape(s_refCountFlagIsStaticString, characterCount - 1, characters,
...
template <unsigned length>
inline bool startsWithLettersIgnoringASCIICase(const StringImpl &string, const char (&lowercaseLetters)[length])
{
...
template <unsigned length>
inline bool equalLettersIgnoringASCIICase(const StringImpl &string, const char (&lowercaseLetters)[length])
{
